...
## Pipeline (batch-aware)

The estate is partitioned into **batches** — one per test class, carrying its
dependency closure — around a **shared core** (page objects, helpers, config,
oracle data). Each batch owns a reserved TIM id range (seq 1 -> TIM-101..,
seq 2 -> TIM-201..) so regenerating one batch never renumbers another.

```
python pipeline.py ingest       1. Selenium adapter -> shared-core.json + ir/batches/* + manifest.json
python pipeline.py batches         List batches: seq, class, TIM range, understood/review status
python pipeline.py understand   2. One Claude call per batch -> tim/batches/* + merged tim-suite.json
python pipeline.py generate     3. Shared assets + per-batch specs -> Playwright framework
...
Every stage auto-discovers all batches from the manifest by default; add
`--batch <id>` to any stage to (re)process just one — batch-scoped runs merge
their results into the shared artifacts without touching other batches, and
reviewer decisions on untouched batches survive partial re-runs.
